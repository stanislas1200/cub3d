...
/*   Updated: 2023/09/23 16:38:31 by dspilleb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void check_for_hit(t_game *game, t_ray *ray)
{
	...
	while (ray->hit == 0 && !stop)
	{
		ray->my = ((int)ray->rx)>>6;
		ray->mx = ((int)ray->ry)>>6;
		if ((ray->my >=0 && ray->mx >=0) && ray->my < game->data->height && ray->mx < ft_strlen(game->data->map[ray->my]) && game->data->map[ray->my][ray->mx])
		{
			if (game->data->map[ray->my][ray->mx]=='X')
			{
				ray->hit = 1;
				ray->dist = distance(game->player.px, game->player.py, ray->rx, ray->ry);
			}
			else
			{
				ray->rx+=ray->xo;
...
void	setup_horizontal_ray(t_ray *ray, t_game *game)
{
	...
	if (sin(deg_to_rad(ray->ra)) < -0.001)// il regarde en bas
	{
	...
	else if (sin(deg_to_rad(ray->ra))> 0.001) // il regarde en haut
	{
	...
	else // il regarde sur le coté 
	{
		ray->rx = game->player.px;
		ray->ry = game->player.py;
		ray->hit = -1; // il ne trouvera jamais
	}
}
...
void	setup_vertical_ray(t_ray *ray, t_game *game)
{
	...
	if (cos(deg_to_rad(ray->ra))> 0.001)// il regarde à gauche
	{
	...
	else if (cos(deg_to_rad(ray->ra))<-0.001) // il regarde à droite
	{
	...
	else // il regarde en haut ou en bas
	{
		ray->ry = game->player.py;
